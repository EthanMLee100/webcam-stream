"""
YOLOv8 Person Livestream + 3s Clip Capture + Backend JWT Upload
"""
...
def convert_to_h264(src_path):
    ...
    if result.returncode != 0:
        print("ffmpeg failed; using original file.")
        return src_path
    return h264_path
...
def main():
    ...
    try:
        while True:
                ...
                if now >= recording_end_ts:
                    ...
                    try:
                        ...
                        if e.response.status_code == 401:
                            jwt_token = fetch_jwt()
                            upload_clip(upload_path, jwt_token, duration)
                            print(f"Re-uploaded {upload_path} after refreshing token")
                        else:
            ...
            annotated = det[0].plot() if det else frame
            cv2.imshow("YOLOv8 Person Livestream", annotated)
